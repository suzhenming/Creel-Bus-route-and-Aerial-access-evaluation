...
TP_NM = "../True_Pop/Simu_2021_LK_ERIE_bus_route_Pop_7.csv"  # Sept
...
# simulation options for sampling
is_simu_pop <- T;
est_by_daytype = T
...
{
  shift_times_airplane = c(7, 19)  # daytime c(7, 19) # all-day  #c(7, 21) same length as interview shift 
} else {
	n_shifts_airplane = 1
	shift_times_airplane = c(7, 19)
}

# Interview shifts
n_shifts_int = 1
shifts_non_overlapping = T  # FALSE
if (n_shifts_int == 1)
{
  shift_times_int = c(4, 23)  # 
} else if (n_shifts_int == 2) 
{
	if (shifts_non_overlapping) {
	  # two shifts
	  shift_times_int = c(5, 15, 23)   #c(7, 15, 23)
	}else{
	  # Two overlapping shifts
      # Shift A: 7:00 AM~3:30 PM  Shift B: 2:30 PM~11:00 PM  
	  # 
	  shift_times_int = c(5, 15, 11, 23)  # c(7, 17, 13, 23) # c(7, 15, 15, 23) 
	}
} else if (n_shifts_int == 3) 
{
  #Three shifts, two are sampled
  shift_times_int = c(4, 10, 16, 23)
} else
	{
	n_shifts_int = 1
...
set.seed(53335);
n_simu <- 5000
for (ncnts in c(1)){  #c(1,2)
  for (ndays_s in c(25)) {  #c(10,15,20,25)
    for (FValue in c(12)) {  #FishableHoursAerial <- 12  # 18  #for July c(10, 11, 12, 13, 18)
      #ncnts=2
      #ndays_s=20
      ...
      if (dailyEst) {
        if (est_by_daytype)
          FN <- paste("res_DailyEst_2k_NOVL_PropCorr_", ndays_s, "_shifts_int", n_shifts_int, "_c", n_shifts_airplane, "_F" , FValue, "_ncnts_" , ncnts, sep="")
        else
