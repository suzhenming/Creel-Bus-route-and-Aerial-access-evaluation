...
f_sample_days <- function(is_simu_pop, trip_pop, days_pop, ndays_s, strat_by_wkday)
{
	if (!strat_by_wkday)
	{
	...
	data.frame(days_s = days_s)
}

f_sample_shifts <- function(is_simu_pop, trip_pop, days_s, ndays_s, n_shifts)
{
  # to-do
...
#  Fishing day: [4 to 22]
    # A shift: 4 to 12
	# B shift: 12 to 22
f_sample_instant_count_times <- function(is_simu_pop = TRUE, ndays_s = 10, ncnts = 1, n_shifts = 2, shifts_s = "O", shift_times = c(4, 22), shifts_non_overlapping = TRUE, AerialProportion)
{
...
#  Fishing day: [4 to 22]
    # A shift: 4 to 12
	# B shift: 12 to 22
f_sample_progressive_count_times <- function(is_simu_pop = TRUE, ndays_s = 10, Progressive_count_interval = 0.5, ncnts = 1, n_shifts = 2, shifts_s = "O", shift_times = c(4, 22), shifts_non_overlapping = TRUE)
{
	...
	i <- 1
	for (d in 1:ndays_s){
		# Start times of counts to be made. Each count will take 'Progressive_count_interval' to finish
		switch(n_shifts,
			# one shift
			{
				H <- SEnd-SStart # duration of shift
				k <- trunc(H/Progressive_count_interval) # max. number of counts that can be made in the shift
				count_times[i, ] <- sort(sample(seq(from = SStart, to = SEnd-Progressive_count_interval, by = Progressive_count_interval), size = ncnts))
			},
			# two shifts
			{
				if (shifts_s[i] == "A")
				{
					count_times[i, ] <- sort(sample(seq(from = AStart, to = AEnd-Progressive_count_interval, by = Progressive_count_interval), size = ncnts))
				}
				
				if (shifts_s[i] == "B")
				{
					count_times[i, ] <- sort(sample(seq(from = BStart, to = BEnd-Progressive_count_interval, by = Progressive_count_interval), size = ncnts))
				}
	        },
			# three shifts
			{
				if (shifts_s[i] == "AB")
				{
					count_times[i, ] <- sort(sample(seq(from = AStart, to = BEnd-Progressive_count_interval, by = Progressive_count_interval), size = ncnts))
				}
				
				if (shifts_s[i] == "BC")
				{
					count_times[i, ] <- sort(sample(seq(from = BStart, to = CEnd-Progressive_count_interval, by = Progressive_count_interval), size = ncnts))
				}
				
				if (shifts_s[i] == "AC")
				{
					AC_time_pop = c(seq(from = AStart, to = AEnd-Progressive_count_interval, by = Progressive_count_interval),
					                 seq(from = CStart, to = CEnd-Progressive_count_interval, by = Progressive_count_interval))
...
#   during the interval 'Progressive_count_interval'
f_make_progressive_counts <- function(simu_trip_pop, days_s, Progressive_count_interval, count_times)
{
	...
	for (d in days_s)
	{
		...
		{
		  # Progressive counts: count of all boats visible in the time interval 'Progressive_count_interval'
		  ...
		  cnt_sample <- trips_intercepted_in_roving_count(ct, Progressive_count_interval, trips_pop_day)
  		  cnt <- nrow(cnt_sample) 
		  ...
		  counts$FSITE[r] = cnt_sample$FSITE[1]
		...
		for (ct in count_times[id, ])
		{
		  ...
		  if (cnt > 0)
		  {
		   ...
		   else
		   {
				...
				counts$FSITE[r] = trips_pop_day$FSITE[1]
...
f_get_incomplete_trip_interviews <- function(opt_int_sampling, simu_trip_pop, max_nints, days_s, n_shifts, shifts_s, shift_times = c(4, 13, 22))
{
  # Roving interviews 
...
make_sample <- function(is_simu_pop, angler_trip_pop, Progressive_counting, RovingInterviewing, ndays_s = 20,
n_shifts = 1, shift_times = c(7, 19), n_shifts_int = 2, shift_times_int = c(4, 13, 22), shifts_non_overlapping = FALSE, AerialProportion,
ncnts = 4, Progressive_count_interval, max_nints = 20, strat_by_wkday, opt_int_sampling)
{

	# Progressive_count_interval == time to take for making a progressive count 
	...
	days <- f_sample_days(is_simu_pop, angler_trip_pop, days_pop, ndays_s, strat_by_wkday)
	# Count shifts
    cnt_shifts <- f_sample_shifts(is_simu_pop, angler_trip_pop, days, ndays_s, n_shifts)	
	# Clerk interview shifts

	int_shifts <- f_sample_shifts(is_simu_pop, angler_trip_pop, days, ndays_s, n_shifts_int)	

	# Select start times for making counts each shift
	if (Progressive_counting) {
	  count_times <- f_sample_progressive_count_times(is_simu_pop, ndays_s, Progressive_count_interval, ncnts, n_shifts, cnt_shifts$shifts, shift_times, shifts_non_overlapping)
	} else { 
	  # Instant counts
      n_shifts_c = n_shifts  #1
      shift_times_c = shift_times #c(5, 13)
	  shifts_c = rep("o", length(cnt_shifts$shifts))
	  shifts_non_overlapping_c = T
	  count_times <- f_sample_instant_count_times(is_simu_pop, ndays_s, ncnts, n_shifts_c, shifts_c, shift_times_c, shifts_non_overlapping_c, AerialProportion)
	}

	# Make counts 
	if (Progressive_counting) {
		counts <- f_make_progressive_counts(angler_trip_pop, days$days_s, Progressive_count_interval, count_times)
	} else {
		counts <- f_make_instant_counts(angler_trip_pop, days$days_s, count_times)
	}

	# interviews
	if (RovingInterviewing) {
		interviews <- f_get_incomplete_trip_interviews(opt_int_sampling, angler_trip_pop, max_nints, days$days_s,  n_shifts_int, int_shifts$shifts, shift_times_int)
	} else {
    interviews <- f_get_completed_trip_interviews(opt_int_sampling, angler_trip_pop, max_nints, days$days_s, n_shifts_int, int_shifts$shifts, shift_times_int)
	}

	list(counts = counts, interviews = interviews)
}
