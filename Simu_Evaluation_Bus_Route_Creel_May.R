...
# True simulated population: for May
TP_NM = "../True_Pop/Simu_2021_LK_ERIE_bus_route_Pop_5.csv"
...
holidays <- as.Date(c(paste(Simu_Year, "-", 5, "-", 29, sep="")))
...
{  
  ...
  assign(FN, f_angler_survey_bs_simu(n_simu, Species, est_by_daytype, True_Pop_Values, 
                                     ...
                                     True_Pop, n_shifts, shift_times, 
                                     !shifts_overlapping, adjust_overlap_prob, 
  ...
  write.table(sum_tab, paste(".\\Results_Bus_route\\May\\summ_tab_", FN, ".txt", sep = ""), sep ="\t", row.names = T)
  write.table(summf, paste(".\\Results_Bus_route\\May\\summ_", FN, ".txt", sep = ""), sep ="\t", row.names = F)
  write.table(simu_res, paste(".\\Results_Bus_route\\May\\", FN, ".txt", sep = ""), col.names = NA, sep ="\t")
}

write.table(res_scenarios, paste(".\\Results_Bus_route\\May\\Results_scenarios_", FN, ".txt", sep = ""), sep ="\t", row.names = F)
