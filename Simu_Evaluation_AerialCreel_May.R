...
setwd("~/OneDrive - State of Michigan DTMB/Documents/MiCreel/ResearchStudy/BusRouteStudy/Simulations/RGenerate_Simu_Aerial")

# True simulated population: Simu_2021_Belleville_Pop.txt
# TP_NM = "../True_Pop/Simu_BusRoute_2021_Pop.csv"  # July
TP_NM = "../True_Pop/Simu_2021_LK_ERIE_bus_route_Pop_5.csv"  # Sept
...
True_Pop_Values <- list(boat_hours = sum(True_Pop$TRIPHOURS), 
                        ...
                        FValue = max(True_Pop$ET)-min(True_Pop$ST))
True_Pop_Values
...
holidays <- as.Date(c(paste(Simu_Year, "-", 5, "-", 31, sep="")))
...
{
  shift_times_airplane = c(7, 15) # for ideal c(4, 21)  # daytime c(7, 18) # all-day  #c(7, 21) same length as interview shift 
} else {
	n_shifts_airplane = 1
	shift_times_airplane = c(7, 18)
}
...
for (ncnts in c(1,2)){  #c(1,2)
  for (ndays_s in c(10,15,20,25)) {  #c(10,15,20,25)
    for (FValue in c(8))   #FishableHoursAerial <- 11  # True F 16.75  #for May c(10, 11, 12, 13, 16)
    {
      #if (AerialProportion) FValue = -1
      ...
      if (dailyEst) {
        if (est_by_daytype)
          FN <- paste("res_DailyEst_2k_PropCorr_", ndays_s, "_shifts_i_", n_shifts_int, "_shifts_c_", n_shifts_airplane, "_F" , FValue, "_ncnts_" , ncnts, sep="")
        else
          FN <- paste("res_DailyEst_sday_shift7-19_", ndays_s, "_shifts_int", n_shifts_int, "_c", n_shifts_airplane,"_F" , FValue,  "_ncnts_" , ncnts, sep="")
      }else{
        if (est_by_daytype)
